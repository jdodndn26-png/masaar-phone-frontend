...
function CameraSection({ section }: { section: ProductSection }) {
  ...
  return (
    <section className="mt-16" dir="rtl">
      ...
      {/* Zoom Footer standalone (when no zoomLevels) */}
      ...
      {privacyDisplay && (
        <InView>
          <motion.div variants={fadeUp} className="mb-4">
            <p className="text-sm sm:text-base font-black text-gray-900 mb-3" dir="rtl">{privacyDisplay.title}</p>
            <div className="flex gap-3 overflow-x-auto scrollbar-hide pb-1">
              {privacyDisplay.images.map((item, i) => (
                <div key={i} className="relative rounded-2xl overflow-hidden shrink-0" style={{ width: "clamp(220px, 60vw, 320px)", aspectRatio: "3/4" }}>
                  <Image src={item.image} alt={item.title} fill className="object-cover" sizes="320px" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />
                  <p className="absolute bottom-0 inset-x-0 p-3 text-white text-[11px] sm:text-xs font-semibold leading-snug">{item.title}</p>
                </div>
              ))}
            </div>
          </motion.div>
